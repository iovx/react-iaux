 ...
 *提代一常用的正则表达的验证
 */
const Validator = function (settings) {
...
Validator.prototype = {
   ...
   *@value 检查的值
   */
  check: function (item, value): boolean {
  ...
  isRegExp: function (item): boolean {
  ...
  //过滤字符串中指定模式的字符串子列
  filter: function (str: string, patt) {
    patt = patt == undefined || patt == "" ? this.pattern.illeagal : patt;
